...
class Eforsyning:
    ...
    def get_latest(self):
        ...
        raw_data = self._get_time_series(
            year=year,
        ...
        for fl in result["ForbrugsLinjer"]["TForbrugsLinje"]:
            ...
            for reading in fl["TForbrugsTaellevaerk"]:
                ...
                if reading["IndexNavn"] == "ENG1":
                    ...
                    if unit == "MWh":
                        multiplier = 1000
                    metering_data["energy-start"] = (
                        self._stof(reading["Start"]) * multiplier
                    )
                    metering_data["energy-end"] = (
                        self._stof(reading["Slut"]) * multiplier
                    )
                    metering_data["energy-used"] = (
                        self._stof(reading["Forbrug"]) * multiplier
                    )
                    metering_data["energy-exp-used"] = (
                        self._stof(fl["ForventetForbrugENG1"]) * multiplier
                    )
                    metering_data["energy-exp-end"] = (
                        self._stof(fl["ForventetAflaesningENG1"]) * multiplier
                    )
